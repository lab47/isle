...
func iterateGroupIndexValue(fn func(*Group) error) lineFunc {
	return func(line []byte) (v any, err error) {
		// wheel:*:0:root
		parts := strings.SplitN(string(line), ":", 4)
		if _, err := strconv.Atoi(parts[2]); err != nil {
		...
		group := &Group{Name: parts[0], Gid: parts[2], Members: members}
...
func LookupGroup(groupFile, groupname string) (*Group, error) {
...
func LookupGroupId(groupFile, id string) (*Group, error) {
	f, err := os.Open(groupFile)
	if err != nil {
		return nil, err
	}
...
func LookupUser(userFile, username string) (*User, error) {
...
func LookupUserId(userFile, uid string) (*User, error) {
	f, err := os.Open(userFile)
	if err != nil {
		return nil, err
	}
...
func LookupAdditionalGroups(groupFile, username string) ([]*Group, error) {
	f, err := os.Open(groupFile)
	if err != nil {
		return nil, err
	}
	...
	fn := func(grp *Group) error {
		if slices.Contains(grp.Members, username) {
			ret = append(ret, grp)
		}
	...
	if _, err := readColonFile(f, iterateGroupIndexValue(fn), 4); err != nil {
		return nil, err
	}
