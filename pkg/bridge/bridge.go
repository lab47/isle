...
func Configure(ctx context.Context, log hclog.Logger, cfg *Config) error {
	...
	*/
	...
	log.Trace("bridge created", "mac", brInterface.Mac)

	netns, err := ns.GetNS(cfg.NetNS)
	if err != nil {
		return fmt.Errorf("failed to open netns %q: %v", cfg.NetNS, err)
	}
	defer netns.Close()

	log.Trace("configuring veth interface", "bridge", brInterface.Name, "container-name", cfg.IfName)

	hostInterface, containerInterface, err := setupVeth(netns, br, cfg.IfName, n.MTU, n.HairpinMode, n.Vlan, n.mac)
	if err != nil {
		return err
	}

	log.Trace("veth created", "host", hostInterface.Name, "container", containerInterface.Name)
	...
	*/
	...
	if isLayer3 {
		...
		*/
		...
		for _, addr := range cfg.IPAM.Addresses {
		...
		log.Trace("configuring interface with ips")

		// Configure the container hardware address and IP address(es)
		if err := netns.Do(func(_ ns.NetNS) error {
			if n.EnableDad {
				_, _ = sysctl.Sysctl(fmt.Sprintf("/net/ipv6/conf/%s/enhanced_dad", cfg.IfName), "1")
				_, _ = sysctl.Sysctl(fmt.Sprintf("net/ipv6/conf/%s/accept_dad", cfg.IfName), "1")
			} else {
				_, _ = sysctl.Sysctl(fmt.Sprintf("net/ipv6/conf/%s/accept_dad", cfg.IfName), "0")
			}
			_, _ = sysctl.Sysctl(fmt.Sprintf("net/ipv4/conf/%s/arp_notify", cfg.IfName), "1")

			// Add the IP to the interface
			if err := ipam.ConfigureIface(cfg.IfName, result); err != nil {
				return err
			}
			return nil
		}); err != nil {
		...
		log.Trace("waiting for bridge to stablize")
		...
		if n.IsGW {
			log.Trace("configuring bridge with gateway addresses")
		...
		if n.IPMasq {
			log.Trace("configuring ipmasq for container")

			chain := utils.FormatChainName(n.Name, cfg.ContainerID)
			comment := utils.FormatComment(n.Name, cfg.ContainerID)
			for _, ipc := range result.IPs {
		...
		if err := netns.Do(func(_ ns.NetNS) error {
			...
			if err = netlink.LinkSetUp(link); err != nil {
				return fmt.Errorf("failed to set %q up: %v", cfg.IfName, err)
			}
			return nil
		}); err != nil {
			return err
...
func Delete(ctx context.Context, log hclog.Logger, cfg *Config) error {
	n := &NetConf{
		NetConf: types.NetConf{
			Name: cfg.NetworkName,
		},
	...
	*/
	...
	err := ns.WithNetNSPath(cfg.NetNS, func(_ ns.NetNS) error {
		var err error
		ipnets, err = ip.DelLinkByNameAddr(cfg.IfName)
