...
const (
	...
	streamPackage  = protogen.GoImportPath("github.com/lab47/isle/pkg/pbstream")

	generatedFilenameExtension = ".pbstream.go"
	generatedPackageSuffix     = "pbstream"
...
func generatePreamble(g *protogen.GeneratedFile, file *protogen.File) {
	...
	g.P("const _ = ", streamPackage.Ident("IsAtLeastVersion0_1_0"))
	g.P()
}
...
func generateClientImplementation(g *protogen.GeneratedFile, service *protogen.Service, names names) {
	clientOption := streamPackage.Ident("ClientOption")
	...
	g.P("func ", names.ClientConstructor, " (client ", streamPackage.Ident("Client"),
		", opts ...", clientOption, ") ", names.Client, " {")
	// g.P("baseURL = ", stringsPackage.Ident("TrimRight"), `(baseURL, "/")`)
	g.P("return &", names.ClientImpl, "{")
	for _, method := range service.Methods {
		g.P(unexport(method.GoName), ": ",
			streamPackage.Ident("NewEndpoint"),
	...
	for _, method := range service.Methods {
		g.P(unexport(method.GoName), " *", streamPackage.Ident("Endpoint"),
			"[", method.Input.GoIdent, ", ", method.Output.GoIdent, "]")
	}
	g.P("}")
	g.P()
...
func clientSignature(g *protogen.GeneratedFile, method *protogen.Method, named bool) string {
	...
	if method.Desc.IsStreamingClient() && method.Desc.IsStreamingServer() {
		...
		return method.GoName + "(" + ctxName + " " + g.QualifiedGoIdent(contextPackage.Ident("Context")) + ") " +
			"*" + g.QualifiedGoIdent(streamPackage.Ident("BidiStreamForClient")) +
			"[" + g.QualifiedGoIdent(method.Input.GoIdent) + ", " + g.QualifiedGoIdent(method.Output.GoIdent) + "]"
	}
	if method.Desc.IsStreamingClient() {
		// client streaming
		return method.GoName + "(" + ctxName + " " + g.QualifiedGoIdent(contextPackage.Ident("Context")) + ") " +
			"*" + g.QualifiedGoIdent(streamPackage.Ident("ClientStreamForClient")) +
			"[" + g.QualifiedGoIdent(method.Input.GoIdent) + ", " + g.QualifiedGoIdent(method.Output.GoIdent) + "]"
	}
	if method.Desc.IsStreamingServer() {
		return method.GoName + "(" + ctxName + " " + g.QualifiedGoIdent(contextPackage.Ident("Context")) +
			", " + reqName + " *" + g.QualifiedGoIdent(streamPackage.Ident("Request")) + "[" +
			g.QualifiedGoIdent(method.Input.GoIdent) + "]) " +
			"(*" + g.QualifiedGoIdent(streamPackage.Ident("ServerStreamForClient")) +
...
func generateServerConstructor(g *protogen.GeneratedFile, service *protogen.Service, names names) {
	...
	handlerOption := streamPackage.Ident("HandlerOption")
	g.P("func ", names.ServerConstructor, "(svc ", names.Server, ", opts ...", handlerOption,
		") (string, ", streamPackage.Ident("Handler"), ") {")
	g.P("mux := ", streamPackage.Ident("NewMux"), "()")
	for _, method := range service.Methods {
		isStreamingServer := method.Desc.IsStreamingServer()
		isStreamingClient := method.Desc.IsStreamingClient()
		switch {
		case isStreamingClient && !isStreamingServer:
			g.P(`mux.Handle("`, procedureName(method), `", `, streamPackage.Ident("NewClientStreamHandler"), "(")
		case !isStreamingClient && isStreamingServer:
			g.P(`mux.Handle("`, procedureName(method), `", `, streamPackage.Ident("NewServerStreamHandler"), "(")
		case isStreamingClient && isStreamingServer:
			g.P(`mux.Handle("`, procedureName(method), `", `, streamPackage.Ident("NewBidiStreamHandler"), "(")
		default:
			g.P(`mux.Handle("`, procedureName(method), `", `, streamPackage.Ident("NewUnaryHandler"), "(")
		}
		g.P(`"`, procedureName(method), `",`)
...
func generateUnimplementedServerImplementation(g *protogen.GeneratedFile, service *protogen.Service, names names) {
	...
	for _, method := range service.Methods {
		...
		if method.Desc.IsStreamingServer() {
			g.P("return ", streamPackage.Ident("NewError"), "(",
				streamPackage.Ident("CodeUnimplemented"), ", ", errorsPackage.Ident("New"),
				`("`, method.Desc.FullName(), ` is not implemented"))`)
		} else {
			g.P("return nil, ", streamPackage.Ident("NewError"), "(",
				streamPackage.Ident("CodeUnimplemented"), ", ", errorsPackage.Ident("New"),
				`("`, method.Desc.FullName(), ` is not implemented"))`)
		}
		g.P("}")
...
func serverSignatureParams(g *protogen.GeneratedFile, method *protogen.Method, named bool) string {
	...
	if method.Desc.IsStreamingClient() && method.Desc.IsStreamingServer() {
		...
		return "(" + ctxName + g.QualifiedGoIdent(contextPackage.Ident("Context")) + ", " +
			streamName + "*" + g.QualifiedGoIdent(streamPackage.Ident("BidiStream")) +
			"[" + g.QualifiedGoIdent(method.Input.GoIdent) + ", " + g.QualifiedGoIdent(method.Output.GoIdent) + "]" +
			") error"
	}
	if method.Desc.IsStreamingClient() {
		// client streaming
		return "(" + ctxName + g.QualifiedGoIdent(contextPackage.Ident("Context")) + ", " +
			streamName + "*" + g.QualifiedGoIdent(streamPackage.Ident("ClientStream")) +
			"[" + g.QualifiedGoIdent(method.Input.GoIdent) + "]" +
			") (*" + g.QualifiedGoIdent(streamPackage.Ident("Response")) + "[" + g.QualifiedGoIdent(method.Output.GoIdent) + "] ,error)"
	}
	if method.Desc.IsStreamingServer() {
		// server streaming
		return "(" + ctxName + g.QualifiedGoIdent(contextPackage.Ident("Context")) +
			", " + reqName + "*" + g.QualifiedGoIdent(streamPackage.Ident("Request")) + "[" +
			g.QualifiedGoIdent(method.Input.GoIdent) + "], " +
			streamName + "*" + g.QualifiedGoIdent(streamPackage.Ident("ServerStream")) +
			"[" + g.QualifiedGoIdent(method.Output.GoIdent) + "]" +
			") error"
	}
	// unary
	return "(" + ctxName + g.QualifiedGoIdent(contextPackage.Ident("Context")) +
		", " + reqName + "*" + g.QualifiedGoIdent(streamPackage.Ident("Request")) + "[" +
		g.QualifiedGoIdent(method.Input.GoIdent) + "]) " +
		"(*" + g.QualifiedGoIdent(streamPackage.Ident("Response")) + "[" +
...
func newNames(service *protogen.Service) names {
	...
	return names{
		Base:                base,
		Client:              fmt.Sprintf("PBS%sClient", base),
		ClientConstructor:   fmt.Sprintf("PBSNew%sClient", base),
		ClientImpl:          unexport(fmt.Sprintf("PBS%sClient", base)),
		Server:              fmt.Sprintf("PBS%sHandler", base),
		ServerConstructor:   fmt.Sprintf("PBSNew%sHandler", base),
		UnimplementedServer: fmt.Sprintf("PBSUnimplemented%sHandler", base),
	}
}
